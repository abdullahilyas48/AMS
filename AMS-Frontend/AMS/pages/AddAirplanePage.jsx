...
const AddAirplanePage = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://192.168.1.113:7798/hangar-reservation/${userData._id}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleAddAirplane = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await fetch('http://192.168.1.113:7798/add-airplane', {
        method: 'POST',
        headers: {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setIsLoading(true);
              const res = await fetch(`http://192.168.1.113:7798/delete-airplane/${airplaneId}`, {
                method: 'DELETE',
              });
