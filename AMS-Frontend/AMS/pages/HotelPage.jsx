...
const HotelBookingPage = () => {
  ...
  const fetchAvailableRooms = async () => {
    ...
    try {
      const response = await axios.get(`http://192.168.1.113:7798/hotel-rooms?minPrice=${priceRange[0]}&maxPrice=${priceRange[1]}`);
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.1.113:7798/hotel-book', payload);
      ...
      <View style={styles.container}>
        ...
        >
          <Text style={styles.blackText}>
            {selectedRoom
              ? `${selectedRoom.hotelName} - ${selectedRoom.roomType} ($ ${selectedRoom.price})`
            ...
            <View style={styles.dropdownList}>
              {rooms.length === 0 ? (
                ...
                rooms.map((room) => (
                  ...
                  >
                    <Text style={styles.blackText}>
                      {`${room.hotelName} - ${room.roomType} ($ ${room.price})`}
                    </Text>
                  </TouchableOpacity>
                ))
              )}
